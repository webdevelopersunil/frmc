    ...
    <div class="container-fluid">
        <div class="collapse navbar-collapse" id="navbarColor01">
            ...
            <ul class="navbar-nav me-auto">
                ...
                <li class="nav-item">
                    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                        <select class="form-select filter-select-css radius-border" name="role" onchange="updateUrl('role', this.value)">
                            ...
                            <option  {{"nodal"  == request()->query('role') ? 'selected' : ''}} value="nodal">NODAL</option>
                            <option  {{"fco"    == request()->query('role') ? 'selected' : ''}} value="fco">FCO</option>
                        </select>
                    </div>
                </li>
                ...
                <li class="nav-item">
                    <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                        <select class="form-select filter-select-css radius-border" name="user" onchange="updateUrl('user', this.value)" >
                        <!-- style="background-color: #08AE72; border-radius:9px; color: white;" -->
                            <option selected disabled value="">Select User</option>
                            @if(isset($users) &&  !empty($users))
                                @foreach($users as $index => $user)
                                    <option {{$user->id  == request()->query('user') ? 'selected' : ''}}  value="{{$user->id}}"> {{ $user->name }} </option>
                                @endforeach
                            @endif
